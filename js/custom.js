$(document).ready(function() {
   ...
   // Mobile dropdown toggles inside sidenav
   $(document).on('click', '.sidenav .nav-item.dropdown > .nav-link', function(e) {
      if ($(window).width() < 992) {
         e.preventDefault();
         var $menu = $(this).siblings('.dropdown-menus');
         $menu.toggleClass('open');
      }
   });

   // FAQ accordion — close others when one opens
   $(document).on('click', '.svc-faq-q', function() {
      var $item = $(this).parent('.svc-faq-item');
      var isOpen = $item.hasClass('open');
      // Close all
      $('.svc-faq-item').removeClass('open');
